...
static const esp_cam_sensor_isp_info_t gc4053_isp_info_mipi[] = {
    {
        .isp_v1_info = {
            ...
            .tline_ns    = 29629,
            /*
             * CRITICAL: GC4053 native MIPI bayer output is GRBG.
             *   The v2 Rockchip Linux reference driver
             *   (gc4053v2.c) declares the format as
...
static const esp_cam_sensor_format_t gc4053_format_info_mipi[] = {
...
#if CONFIG_CAMERA_GC4053_MIPI_RAW8_1920X1080_25FPS
    {
        ...
        .width = 1440,
        .height = 1440,
        .regs = init_reglist_MIPI_2lane_24Minput_RAW10_1440x1440_30fps,
        .regs_size = ARRAY_SIZE(init_reglist_MIPI_2lane_24Minput_RAW10_1440x1440_30fps),
        .fps = 30,
        .isp_info = &gc4053_isp_info_mipi[1],
        .mipi_info = {
            .mipi_clk = 972000000,
            .lane_num = 2,
            .line_sync_en = false,
...
static esp_err_t gc4053_set_reg_bits(esp_sccb_io_handle_t sccb_handle, uint16_t reg, uint8_t offset, uint8_t length, uint8_t value)
{
    esp_err_t ret = ESP_OK;
    uint8_t reg_data = 0;

    ret = gc4053_read(sccb_handle, reg, &reg_data);
    if (ret != ESP_OK) {
        return ret;
    }
    uint8_t mask = ((1 << length) - 1) << offset;
    value = (reg_data & ~mask) | ((value << offset) & mask);
    ret = gc4053_write(sccb_handle, reg, value);
    return ret;
}
...
static esp_err_t gc4053_set_total_gain_val(esp_cam_sensor_device_t *dev, uint32_t u32_val)
{
    ...
    if (u32_val >= cam_gc4053->gc4053_para.limited_abs_gain_index) {
        if (cam_gc4053->gc4053_para.limited_abs_gain_index > 0) {
            u32_val = cam_gc4053->gc4053_para.limited_abs_gain_index - 1;
        } else {
