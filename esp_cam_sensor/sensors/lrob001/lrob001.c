...
#if !CONFIG_CAMERA_LROB001_MIPI_RAW10_736X336_296FPS
...
#if CONFIG_CAMERA_LROB001_MIPI_RAW10_736X336_296FPS
    {
        .name = "MIPI_2lane_24Minput_RAW10_736x336_296fps",
        .format = ESP_CAM_SENSOR_PIXFORMAT_RAW10,
        .port = ESP_CAM_SENSOR_MIPI_CSI,
        .xclk = 24000000,
        .width = 736, // 368x2
        .height = 336, // 7x48=336, 7x47 = 329 work
        .regs = lrob001_cfg_raw10,
        .regs_size = ARRAY_SIZE(lrob001_cfg_raw10),
        .fps = 296,
...
static esp_err_t lrob001_write_block(esp_sccb_io_handle_t sccb_handle, uint16_t reg, const uint8_t *data, size_t data_len)
{
    return esp_sccb_transmit_block_a16(sccb_handle, reg, data, data_len);
}

static esp_err_t lrob001_write_array_raw10(esp_sccb_io_handle_t sccb_handle, lrob001_reginfo_t *regarray, size_t regs_size)
{
    int i = 0;
    esp_err_t ret = ESP_OK;
    while ((ret == ESP_OK) && (i < regs_size)) {
        if (regarray[i].type == CFG_TYPE_WRITE) {
            ret = lrob001_write(sccb_handle, regarray[i].reg_addr, regarray[i].reg_value);
        } else if (regarray[i].type == CFG_TYPE_BLOCK_WRITE) {
            ret = lrob001_write_block(sccb_handle, regarray[i].reg_addr, lrob001_roi_data_raw10, ARRAY_SIZE(lrob001_roi_data_raw10));
        } else {
            esp_rom_delay_us(regarray[i].delay_us);
        }
        i++;
    }
...
static esp_err_t lrob001_write_array_raw8(esp_sccb_io_handle_t sccb_handle, lrob001_reginfo_t *regarray, size_t regs_size)
{
    int i = 0;
    esp_err_t ret = ESP_OK;
    while ((ret == ESP_OK) && (i < regs_size)) {
        if (regarray[i].type == CFG_TYPE_WRITE) {
            ...
            ret = lrob001_write_block(sccb_handle, regarray[i].reg_addr, lrob001_roi_data_raw8, ARRAY_SIZE(lrob001_roi_data_raw8));
        } else {
            esp_rom_delay_us(regarray[i].delay_us);
        }
        i++;
...
static esp_err_t lrob001_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    ...
    ret = lrob001_write_array_raw10(dev->sccb_handle, (lrob001_reginfo_t *)format->regs, format->regs_size);
...
static esp_err_t lrob001_set_test_pattern(esp_cam_sensor_device_t *dev, int enable)
{
    return lrob001_write(dev->sccb_handle, 0x0011, enable ? 0x01 : 0x00);
}
...
static esp_err_t lrob001_priv_ioctl(esp_cam_sensor_device_t *dev, uint32_t cmd, void *arg)
{
    ...
    case ESP_CAM_SENSOR_IOC_S_TEST_PATTERN:
        ret = lrob001_set_test_pattern(dev, *(int *)arg);
        break;
    default:
        break;
    }
