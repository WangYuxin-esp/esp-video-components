...
static const uint8_t s_mira220_exp_max_offset = 0xb; // max exposure = (height + vblank) - (int)(MIRA220_GLOB_NUM_CLK_CYCLES / row_length);
static const char *TAG = "mira220";

/* Mira220 does not support analog gain. */
static const uint32_t mira220_total_gain_val_map[] = {
    1000,
    1001,
    1002,
};
...
static const esp_cam_sensor_isp_info_t mira220_isp_info[] = {
    /* For MIPI */
    {
        .isp_v1_info = {
            ...
            .pclk = 25190400,
            .vts = 4193,  // Vblank + Vsize = {0x1012, 0x1013} + {0x1087, 0x1088}
            .hts = 300, // 0x102b, 0x102c
            .tline_ns = 2757, // todo, update.
            .gain_def = 0,
            .exp_def = 0x80, // 0x100c, 0x100d
...
static const esp_cam_sensor_format_t mira220_format_info[] = {
    /* For MIPI */
    {
        .name = "MIPI_2lane_RAW12_1280_720_15fps",
        .format = ESP_CAM_SENSOR_PIXFORMAT_RAW8,
        .port = ESP_CAM_SENSOR_MIPI_CSI,
        .xclk = 38400000,
        .width = 1280,
        .height = 720,
        .regs = init_reglist_MIPI_2lane_1280_720_15fps,
        .regs_size = ARRAY_SIZE(init_reglist_MIPI_2lane_1280_720_15fps),
        .fps = 15,
...
/* Please ensure LSB is written before the MSB when mutil address is used */
...
static esp_err_t mira220_set_exp_val(esp_cam_sensor_device_t *dev, uint32_t u32_val)
{
    ...
    // Master mode is used default, T(frame) = T(clk_in) * ROW_LEngth * (VSIZE + VBLANK)
    ...
    ret = mira220_write(dev->sccb_handle, MIRA220_REG_EXPOSURE_L, (value_buf & 0xff));
    ret |= mira220_write(dev->sccb_handle, MIRA220_REG_EXPOSURE_H, (value_buf >> 8) & 0xff);
...
static esp_err_t mira220_query_para_desc(esp_cam_sensor_device_t *dev, esp_cam_sensor_param_desc_t *qdesc)
{
    ...
    case ESP_CAM_SENSOR_EXPOSURE_VAL:
        ...
        qdesc->number.maximum = dev->cur_format->isp_info->isp_v1_info.vts - s_mira220_exp_max_offset; // max = VTS-6 = height+vblank-6, so when update vblank, exposure_max must be updated
        qdesc->number.step = 1;
    ...
    case ESP_CAM_SENSOR_EXPOSURE_US:
        ...
        qdesc->number.maximum = EXPOSURE_MIRA220_TO_V4L2((dev->cur_format->isp_info->isp_v1_info.vts - s_mira220_exp_max_offset), dev->cur_format); // max = VTS-6 = height+vblank-6, so when update vblank, exposure_max must be updated
    ...
    case ESP_CAM_SENSOR_GROUP_EXP_GAIN:
        (void)mira220_total_gain_val_map;
...
static esp_err_t mira220_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    ...
    cam_mira220->mira220_para.exposure_max = dev->cur_format->isp_info->isp_v1_info.vts - s_mira220_exp_max_offset;
...
esp_cam_sensor_device_t *mira220_detect(esp_cam_sensor_config_t *config)
{
    ...
    if (mira220_get_sensor_id(dev, &dev->id) != ESP_OK) {
        ...
        goto err_free_handler;
    } else if ((dev->id.pid != MIRA220_PID) && (dev->id.pid != 0x258)) {
