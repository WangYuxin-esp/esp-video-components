...
static ColorDetect *s_color_detect;
static ColorRotateDetect *s_rot_detect;
...
static esp_err_t image_color_detection_process(uint8_t *src1_buf, uint32_t src_width, uint32_t src_height)
{
    ...
    auto &detect_results = s_color_detect->run(color1);
    for (const auto &res : detect_results) {
        ESP_LOGI("ColorDetect",
                 "[name: %s, [x1: %d, y1: %d, x2: %d, y2: %d]]",
                 s_color_detect->get_color_name(res.category).c_str(),
                 res.box[0],
                 res.box[1],
                 res.box[2],
                 res.box[3]);
...
static void video_stream_task(void *arg)
{
    ...
    while (1) {
        ...
        image_color_detection_process(s_app_camera_video.camera_buffer[v4l2_buf0_index], s_app_camera_video.camera_buf_hes, s_app_camera_video.camera_buf_ves);
...
extern "C" void app_main(void)
{
    ...
    s_color_detect = new ColorDetect(s_app_camera_video.camera_buf_hes, s_app_camera_video.camera_buf_ves);
    s_color_detect->register_color({170, 100, 100}, {10, 255, 255}, "red");
    s_color_detect->register_color({20, 100, 100}, {35, 255, 255}, "yellow");
    s_color_detect->register_color({35, 100, 100}, {85, 255, 255}, "green");
    s_color_detect->register_color({100, 100, 100}, {130, 255, 255}, "blue");
    s_rot_detect = new ColorRotateDetect(s_app_camera_video.camera_buf_hes, s_app_camera_video.camera_buf_ves);
    s_rot_detect->register_color({170, 100, 100}, {10, 255, 255}, "red");
    s_rot_detect->register_color({20, 100, 100}, {35, 255, 255}, "yellow");
    s_rot_detect->register_color({35, 100, 100}, {85, 255, 255}, "green");
    s_rot_detect->register_color({100, 100, 100}, {130, 255, 255}, "blue");
